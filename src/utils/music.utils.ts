...
export class MusicUtils {
	...
	public static async paginateQueue(
		...
		const titlePages = slicesArray.map((songChunk, i) => {
			const embed = new EmbedBuilder()
				...
				.setTimestamp()
				.setFooter({
					text: `Page ${i + 1} of ${queue.songs.length + slicesArray.length} | Songs starts at page ${slicesArray.length + 1}`
				})
				.addFields(songChunk)
