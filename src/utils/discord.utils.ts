...
export class DiscordUtils {
	public static async replyOrFollowUp(
		...
		CommandInteraction<CacheType> | MessageComponentInteraction<CacheType>
	> {
	...
	public static async joinIfVoiceChannel(
	...
	private static async handleErrorReply(
		...
		await DiscordUtils.replyOrFollowUp(interaction, replyOptions)
		setTimeout(() => interaction.deleteReply(), 15e3)
	}
	...
	public static async handleInteractionError(
		...
		if (error instanceof DiscordAPIError) {
			await DiscordUtils.handleErrorReply(
				interaction,
		...
		if (error instanceof DisTubeError) {
			await DiscordUtils.handleErrorReply(
				interaction,
				`> **MusicPlayerError**: ${error.message}`
			)
			throw error
		}
	...
	public static getInteractionMember(
		interaction: CommandInteraction
	): GuildMember {
		return (
			interaction.member
