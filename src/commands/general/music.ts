...
export class Music {
	...
	async play(
		...
		try {
			const voiceChannel =
				await DiscordUtils.joinIfVoiceChannel(interaction)
	...
	async pause(interaction: CommandInteraction): Promise<void> {
		try {
			...
			await DiscordUtils.replyOrFollowUp(
				interaction,
				`> Paused music: **${queue?.songs[0].name}**`
			)
		} catch (err) {
			DiscordUtils.handleInteractionError(interaction, err)
		}
	...
	async resume(interaction: CommandInteraction): Promise<void> {
		try {
			...
			await DiscordUtils.replyOrFollowUp(
				interaction,
				`> Resumed music: **${queue?.songs[0].name}**`
			)
		} catch (err) {
			DiscordUtils.handleInteractionError(interaction, err)
		}
	...
	async addRelatedSong(interaction: CommandInteraction): Promise<void> {
		try {
			const voiceChannel =
				await DiscordUtils.joinIfVoiceChannel(interaction)
			if (!voiceChannel) return
			...
			const song = await this.musicManager.player.addRelatedSong(
				interaction.guildId
			)
			await DiscordUtils.replyOrFollowUp(
				interaction,
				`> Added related song to queue: **${song.name}**`
			)
		} catch (err) {
			DiscordUtils.handleInteractionError(interaction, err)
		}
	...
	async jump(
		...
		try {
			...
			const song = await this.musicManager.player.jump(
				interaction.guildId,
				num
			)
			await DiscordUtils.replyOrFollowUp(
				interaction,
				`> Jumped to song: **${song.name}**`
			)
		} catch (err) {
			DiscordUtils.handleInteractionError(interaction, err)
		}
	...
	async previous(interaction: CommandInteraction): Promise<void> {
		try {
			...
			const song = await this.musicManager.player.previous(
				interaction.guildId
			)
			await DiscordUtils.replyOrFollowUp(
				interaction,
				`> Playing previous song: **${song.name}**`
			)
		} catch (err) {
			DiscordUtils.handleInteractionError(interaction, err)
		}
	...
	async setRepeatMode(
		...
		try {
			...
			const repeatMode = this.musicManager.player.setRepeatMode(
				interaction.guildId,
				mode
			)
			const finalMode = repeatMode
				? repeatMode === 2
					? 'Repeat queue'
					: 'Repeat song'
				: 'Off'
	...
	async shuffle(interaction: CommandInteraction): Promise<void> {
		try {
			...
			const queue = await this.musicManager.player.shuffle(
				interaction.guildId
			)
			await DiscordUtils.replyOrFollowUp(
				interaction,
				`> Queue shuffled: **${queue.voiceChannel?.guild.name} | ${queue.voiceChannel?.name}**`
			)
		} catch (err) {
			DiscordUtils.handleInteractionError(interaction, err)
		}
	...
	async skip(interaction: CommandInteraction): Promise<void> {
		try {
			...
			const song = await this.musicManager.player.skip(
				interaction.guildId
			)
			await DiscordUtils.replyOrFollowUp(
				interaction,
				`> Song skipped. Playing: **${song.name}**`
			)
		} catch (err) {
			DiscordUtils.handleInteractionError(interaction, err)
		}
	...
	async toggleAutoplay(interaction: CommandInteraction): Promise<void> {
		try {
			const voiceChannel =
				await DiscordUtils.joinIfVoiceChannel(interaction)
			if (!voiceChannel) return
			...
			const autoplay = this.musicManager.player.toggleAutoplay(
				interaction.guildId
			)
			await DiscordUtils.replyOrFollowUp(
				interaction,
